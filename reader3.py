...
def optimize_image(image_data: bytes, max_width: int = 800, quality: int = 65) -> bytes:
    """
...
def process_epub(epub_path: str, output_dir: str) -> Book:
    ...
    for item in book.get_items():
        if item.get_type() == ebooklib.ITEM_IMAGE:
            ...
            # Save to disk with optimization
            local_path = os.path.join(images_dir, safe_fname)
            
            # Optimize image
            original_data = item.get_content()
            optimized_data = optimize_image(original_data)
            
            with open(local_path, 'wb') as f:
                f.write(optimized_data)
            
            # Log compression ratio
            compression_ratio = (1 - len(optimized_data) / len(original_data)) * 100
            if compression_ratio > 5:  # Only log if significant compression
                print(f"  Optimized {safe_fname}: {len(original_data)//1024}KB -> {len(optimized_data)//1024}KB ({compression_ratio:.1f}% reduction)")
